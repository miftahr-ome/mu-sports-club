...
Route::get('/', function () {
    ...
    $users  = DB::table('users')
        ->select('id','name','email','system_role','committee_role','phone','profile_picture')
        ->get();
...
Route::post('/register-member', function (Request $request) {
    try {
        ...
        return response()->json([
            'success' => true,
            'message' => 'Welcome to MU Sports Club!',
        ]);
    } catch (\Exception $e) {
        return response()->json([
            'success' => false,
...
Route::get('/news', function () {
    try {
        ...
        // On error, return empty list but keep success true so UI doesn’t break
        return response()->json([
            'success' => true,
            'data'    => [],
        ]);
...
// ── Small helper for admin auth (inline, no global function) ─────────────────

function isAdminRequest(Request $request): bool
{
...
Route::get('/admin/team-registrations', function (Request $request) {
    ...
    $teams = DB::table('team_registrations')
        ->orderBy('created_at', 'desc')
        ->get();
...
Route::get('/admin/tournament-games', function (Request $request) {
    ...
    $games = DB::table('tournament_games')->get();
